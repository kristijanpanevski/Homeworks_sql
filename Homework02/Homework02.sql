...
--Show all details for artists born before 01
...
SELECT * FROM song s 
WHERE s.duration > interval '4 minutes';
...
--Show albums that have ‘u’ in their name sorted by rating, with the worst rating on top
SELECT * FROM album a 
WHERE a.name LIKE '%u%' 
...
FROM artist a
union all
SELECT ad.full_name AS artist_full_name 
...
--Show all album names and their ratings
SELECT name, rating 
FROM album;

--Show all artists with their name and full name side by side
SELECT a.name as artist_name, ad.full_name as artist_full_name
FROM artist a
join artist_details ad on a.id = ad.artist_id;

--Show all songs with their lyrics side by side
SELECT s.name as song_name, sl.lyrics as song_lyrics
FROM song s
join song_lyrics sl on s.id = sl.song_id;

--Show the artist names and their spouse names for all artists including ones that don’t have details
...
SELECT a.name AS artist_name, ad.country 
FROM artist a 
FULL JOIN artist_details ad 
ON a.id = ad.artist_id;
...
SELECT al.name AS album_name, al.rating, a.name AS artist_name
FROM album al
JOIN song s 
ON al.id = s.album_id
JOIN artist a 
